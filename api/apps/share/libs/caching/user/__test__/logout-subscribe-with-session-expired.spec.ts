...
describe('logout subscribe with session expired', () => {
  it('logout subscribe with session expired success', async () => {
  ...
  it('logout subscribe with session expired subscribe failed with unknown error', async () => {
    expect.hasAssertions();
    const subscriber = {
      connect: jest.fn().mockResolvedValue({}),
