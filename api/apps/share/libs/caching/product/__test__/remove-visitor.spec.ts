...
describe('remove visitor', () => {
  it('remove visitor success', async () => {
    ...
    await expect(productCachingService.removeVisitor(product.product_id as string, user.user_id)).resolves.toBe(result);
    expect(sRem).toHaveBeenCalledTimes(1);
    expect(sRem).toHaveBeenCalledWith(key, user.user_id);
  });
  ...
  it('remove visitor failed with unknown error', async () => {
    ...
    await expect(productCachingService.removeVisitor(product.product_id as string, user.user_id)).rejects.toThrow(
      UnknownError,
    );
    expect(sRem).toHaveBeenCalledTimes(1);
    expect(sRem).toHaveBeenCalledWith(key, user.user_id);
